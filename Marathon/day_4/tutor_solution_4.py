...
class Author:
    ...
    def __repr__(self):
        return "Author({}, {}, {})".format(
            self.first_name, self.second_name, self.year_of_birth
        )
...
class Book:
    def __init__(
        ...
        *authors: Author,
        ...
        if not authors:
            raise ValueError(
                "Author is not specified. At least one author should be specified."
            )
    ...
    def __str__(self):
        return "{}: {}".format(
            self.title, ", ".join(str(author) for author in self.authors)
        )
